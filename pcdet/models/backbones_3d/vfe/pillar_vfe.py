...
class Radar7PillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **args):
        super().__init__(model_cfg=model_cfg)
...
class RadarPillarVFE(VFETemplate):
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **args):
        ...
        ## check if config has the correct params, if not, throw exception
        radar_config_params = ["USE_RCS", "USE_VR", "USE_VR_COMP", "USE_TIME", "USE_ELEVATION", "USE_VR_COMP_XY"]
        # radar_config_params = ["USE_RCS", "USE_VR", "USE_VR_COMP", "USE_TIME", "USE_ELEVATION"]
        ...
        if all(hasattr(self.model_cfg, attr) for attr in radar_config_params):
            ...
            self.use_time = self.model_cfg.USE_TIME
            self.use_elevation = self.model_cfg.USE_ELEVATION
            self.use_vr_comp_xy = self.model_cfg.USE_VR_COMP_XY

        else:
            raise Exception("config does not have the right parameters, please use a radar config")

        self.available_features = ['x', 'y', 'z', 'rcs', 'v_r', 'v_r_comp', "v_r_comp_x", "v_r_comp_y", 'time']
        # 点云特征
        num_point_features += 10  # center_x, center_y, center_z, mean_x, mean_y, mean_z, mean_vr, mean_vr_comp, mean_vr_comp_x, mean_vr_comp_y
        # 特征的索引
        self.x_ind = self.available_features.index('x')
        self.y_ind = self.available_features.index('y')
        self.z_ind = self.available_features.index('z')
        ...
        self.vr_ind = self.available_features.index('v_r')
        self.vr_comp_ind = self.available_features.index('v_r_comp')
        self.vr_comp_x_ind = self.available_features.index("v_r_comp_x")
        self.vr_comp_y_ind = self.available_features.index("v_r_comp_y")
        self.time_ind = self.available_features.index('time')
        ...
        if self.use_vr_comp_xy:
            num_point_features += 2
            self.selected_indexes.extend([self.vr_comp_x_ind, self.vr_comp_y_ind])
    ...
    def forward(self, batch_dict, **kwargs):
        ...
        voxel_features, voxel_num_points, coords = batch_dict['voxels'], batch_dict['voxel_num_points'], batch_dict[
            'voxel_coords']
        # (N, num_points, num_features), # (N) # (N, 4)

        # 速度特征
        orig_vr = voxel_features[:, :, self.vr_ind] # (N, num_points)
        orig_vr_comp = voxel_features[:, :, self.vr_comp_ind] # (N, num_points)
        orig_vr_comp_x = voxel_features[:, :, self.vr_comp_x_ind] # (N, num_points)
        orig_vr_comp_y = voxel_features[:, :, self.vr_comp_y_ind] # (N, num_points)
        # orig_vr_comp_x = np.cos(np.arctan(orig_xyz[:, :, 1] / orig_xyz[:, :, 0])) * orig_vr_comp
        # orig_vr_comp_y = np.sin(np.arctan(orig_xyz[:, :, 1] / orig_xyz[:, :, 0])) * orig_vr_comp
        vr_mean = orig_vr.sum(dim=1, keepdim=True) / voxel_num_points.type_as(voxel_features).view(-1, 1)
        vr_cluster = (orig_vr - vr_mean)[:, :, None] # (N, num_points, 1)
        vr_comp_mean = orig_vr_comp.sum(dim=1, keepdim=True) / voxel_num_points.type_as(voxel_features).view(-1, 1)
        vr_comp_cluster = (orig_vr_comp - vr_comp_mean)[:, :, None]
        vr_comp_x_mean = orig_vr_comp_x.sum(dim=1, keepdim=True) / voxel_num_points.type_as(voxel_features).view(-1, 1)
        vr_comp_x_cluster = (orig_vr_comp_x - vr_comp_x_mean)[:, :, None]
        vr_comp_y_mean = orig_vr_comp_y.sum(dim=1, keepdim=True) / voxel_num_points.type_as(voxel_features).view(-1, 1)
        vr_comp_y_cluster = (orig_vr_comp_y - vr_comp_y_mean)[:, :, None]
        
        # xyz特征
        orig_xyz = voxel_features[:, :, :self.z_ind + 1]  # selecting x y z # (N, num_points, 3)
        ...
        features = [voxel_features, f_cluster, f_center, vr_cluster, vr_comp_cluster, vr_comp_x_cluster, vr_comp_y_cluster]
